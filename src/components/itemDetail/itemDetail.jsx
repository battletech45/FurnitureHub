      ...
      <section className="item-detail-container">
        ...
        <aside className="item-info">
          ...
          <div className="size-choise">
            <p className={`size ${selectedSize === 'XS' ? 'selected' : ''}`}
              onClick={() => handleSizeClick('XS')}>XS</p>
            <p className={`size ${selectedSize === 'L' ? 'selected' : ''}`}
              onClick={() => handleSizeClick('L')}>L</p>
            <p className={`size ${selectedSize === 'XL' ? 'selected' : ''}`}
              onClick={() => handleSizeClick('XL')}>XL</p>
          </div>
          <h5>Color</h5>
          <div className="color-selector">
            <div
              className={`color-circle ${selectedColor === '#816DFA' ? 'colorSelected' : ''}`}
              style={{ backgroundColor: '#816DFA' }}
              onClick={() => handleColorClick('#816DFA')}
            ></div>
            <div
              className={`color-circle ${selectedColor === 'black' ? 'colorSelected' : ''}`}
              style={{ backgroundColor: 'black' }}
              onClick={() => handleColorClick('black')}
            ></div>
            <div
              className={`color-circle ${selectedColor === '#CDBA7B' ? 'colorSelected' : ''}`}
